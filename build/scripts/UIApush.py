...
base_path = "/home/builder/wa/msvn/release/packaging/snapshot_packages"
...
tmp_path_32 = tempfile.mkdtemp(prefix="UIA_")
tmp_path_64 = tempfile.mkdtemp(prefix="UIA_")
...
def Cleanup():
    ...
    rmcmd = 'rm -r %s' %tmp_path_32
    c.getoutput(rmcmd)
    rmcmd = 'rm -r %s' %tmp_path_64
    c.getoutput(rmcmd)
...
def RunTests():
    ...
    results = c.getoutput('ssh qa@uiaqa.sled.lab.novell.com python /var/qa/code/test/harness/remote_run.py -s')
    if(results == 0):
        print "Running Regression Tests"
        c.getoutput('ssh qa@uiaqa.sled.lab.novell.com python /var/qa/code/test/harness/remote_run.py')
...
def SendNotifications():
    ...
    #cmdstr = "mailx stshaw@novell.com -s 'New RPMS - http://build1.sled.lab.novell.com/uia/current' -c bgmerrell@novell.com -c rawang@novell.com -c cachen@novell.com -c ngao@novell.com < %s/rpm_revs" % tmp_path_32
    #c.getoutput(cmdstr)
    subprocess.call(['mailx', 'stshaw@novell.com', '-s', 'New RPM (32 and 64)'], stdin=open('/tmp/UIAoutput'))
#    mailcmd = "mailx stshaw@novell.com -s 'New RPM (32 and 64)' < /tmp/UIAoutput"
 #   c.getoutput(mailcmd)
