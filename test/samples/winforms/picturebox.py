...
class PictureBoxSample(Form):
    ...
    toggle = True
    path_to_file = "%s/winforms/desktop-blue_soccer400x500.jpg" % uiaqa_path
    image = None
    ...
    def __init__(self):
        ...
        self.button.Image = Image.FromFile("%s/winforms/opensuse60x38.gif" % uiaqa_path)
    ...
    def on_click(self, sender, event):
        if self.toggle == True:
            self.toggle = False
            self.path_to_file = "%s/winforms/universe300x400.jpg" % uiaqa_path
            self.change_picture()
            self.label.Text = "show universe300x400.jpg"
        else:
            self.toggle = True
            self.path_to_file = "%s/winforms/desktop-blue_soccer400x500.jpg" % uiaqa_path
            self.change_picture()
            self.label.Text = "show desktop-blue_soccer400x500.jpg"
