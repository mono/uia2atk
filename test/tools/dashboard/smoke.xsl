            ...
            <div class="header">
                <p>
                    <span id="documentName">
                        <xsl:value-of select="$control"/> Smoke Test
                    </span>
                </p>
            </div>
            <div class="subheader">
                <p>
                    <span id="lastUpdated">
                        Last Updated: <xsl:value-of select="smoke/timeAndDate"/>
                    </span>
                </p>
            </div>
            <div class="smoke">
                <xsl:comment>Convert the number of seconds it took the smoke test
                    to run into days, hours, minutes, and seconds</xsl:comment>
                <xsl:variable name="numDays" select="floor(number(smoke/elapsedTime) div 86400)"/>
                <xsl:variable name="tmpSec1" select="number(smoke/elapsedTime) mod 86400"/>
                ...
                <xsl:variable name="numSeconds" select="format-number(number(smoke/elapsedTime) mod 60, '#.#')"/>
                <p>
                ...
                <table>
                    ...
                    <xsl:for-each select="smoke/machine">
                        <tr>
                            <td><xsl:number/></td>
                            <xsl:variable name="name" select="name"/>
                            <td>
                                <a href="../../logs/smoke/{$controlLower}/{$name}">
